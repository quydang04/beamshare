...
import BeamShareServer from "./server.js";
import BeamShareWsServer from "./ws-server.js";
...
if (conf.signalingServer) {
    ...
    if (!isValidUrl || containsProtocol) {
        console.error("SIGNALING_SERVER must be a valid url without the protocol prefix.\n" +
            "Examples of valid values: `beamshare.net`, `beamshare.example.com:3000`, `example.com/beamshare`");
        process.exit(1);
    }
...
// Start a new BeamShare instance when an uncaught exception occurs
if (conf.autoStart) {
    process.on(
        'uncaughtException',
        () => {
...
const beamShareServer = new BeamShareServer(conf);

if (!conf.signalingServer) {
    // Start websocket server if SIGNALING_SERVER is not set
    new BeamShareWsServer(beamShareServer.server, conf);
} else {
...
console.log('\nBeamShare is running on port', conf.port);
